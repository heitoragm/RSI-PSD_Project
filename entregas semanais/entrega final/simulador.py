...
if len(sys.argv) != 2:
	print >> sys.stderr, "Usage: %s [simulador.py Runtime]" % (sys.argv[0])
	sys.exit(1)
else:
	try:
...
def conteudoMap(i):
	map(lambda x: listaTempo.append(x[0]),pcap.pcap(name='./probes/probes-2013-03-30.pcap'+str(i)))
map(conteudoMap,range(0,7))
tempoEsperado = (listaTempo[-1]-listaTempo[0])
...
for i in range(7):
	...
	for ts, pkt in pcap.pcap(name='./probes/probes-2013-03-30.pcap'+str(i)):
		...
		if wifi.type == 0 and wifi.subtype == 4:
			if nPkts != 0:
				intervaloProbes = listaTempo[nPkts]-listaTempo[nPkts-1]
				...
				try:
					if (atraso > (intervaloProbes/runtime - (timeit.default_timer()-startProbe))):
						atraso -= (intervaloProbes/runtime - (timeit.default_timer()-startProbe))
						startProbe = timeit.default_timer()
					else:
						time.sleep((intervaloProbes/runtime - (timeit.default_timer()-startProbe)-atraso))
						startProbe = timeit.default_timer()
						atraso = 0
				except:
					atraso -= (intervaloProbes/runtime - (timeit.default_timer()-startProbe))
