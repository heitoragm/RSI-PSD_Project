#bin/spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.4.3 /home/rsi-psd-vm/Documents/rsi-psd-codes/psd/pratica-05/ConsultasDados/Dispositivos.py localhost:9092 subscribe rsipsd_project
...
def processRow(row):
    print(row)
    row_data = { 'dispositivosTabela' : row.__getitem__("count")}
...
if __name__ == "__main__":
    ...
    dispositivos = probes.select(
        approx_count_distinct('mac',rsd = 0.01).alias('count')
    )
    query = dispositivos\
        .writeStream\
        .outputMode("complete")\
