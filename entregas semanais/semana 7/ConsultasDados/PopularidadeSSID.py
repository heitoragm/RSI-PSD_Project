...
if __name__ == "__main__":
    ...
    # popularidadeSSID = popularidadeSSID.select( popularidadeSSID.count )

    # dispositivosPorSSID = popularidadeSSID.select(
    #     popularidadeSSID.count
    # )
    #incompleto
    query = popularidadeSSID.select((popularidadeSSID.SSID+', '+str(popularidadeSSID.__getitem__("count"))).alias('value'))\
        .selectExpr("CAST(value AS STRING)") \
        .writeStream\
        .format("kafka") \
        .outputMode("complete") \
